...
class HesaiPcapParser {
  ...
  Future<void> readPcap() async {
    ...
    for (var i = 0; i < content.length - packetWithHeaderLen-1; i += packetWithHeaderLen) {
      for (var point in parsePacketBody(contentNoHeader.sublist(i + 16 + 42, i + packetWithHeaderLen))) {
  ...
  Iterable<VeloPoint> parsePacketBody(Uint8List packetBody) sync* {
    ...
    for (int blockIndex = 0; blockIndex < blockNum; blockIndex++) {
      final blockTimestamp = _calcBlockTimestamp(dateTime, timestamp, blockIndex, returnMode);
  ...
  int _calcBlockTimestamp(Uint8List dateTime, int timestamp, int blockId, int returnMode) {
  ...
  (double, double, double) _calcPolarCoordinate(double azimuthDeg, double vAngleDeg, double distanceM) {
  ...
  Iterable<VeloPoint> _parseBlock(Uint8List packetBlock, int blockTimestamp) sync* {
    ...
    for (int channel = 0; channel < 32; channel++) {
      ...
      final (x,y,z) = _calcPolarCoordinate(azimuth / 100.0, vAngle.toDouble(), distance * 4.0 / 1000.0);
      yield VeloPoint(
        reflectivity,
        channel,
        azimuth,
