...
pub struct PcdFragment {
    pub vertices: Vec<f32>,
    pub frame_start_indices: Vec<u32>,
    pub max_point_num: u32,
}

pub fn read_pcap_stream(stream: StreamSink<PcdFragment>, path: String, frames_per_fragment: u32) {
    let start = std::time::Instant::now();
    let file = File::open(path).unwrap();
    let mut reader = LegacyPcapReader::new(65536, file).unwrap();

    let mut writer = VertexWriter::create(frames_per_fragment, stream);
    loop {
        match reader.next() {
            Ok((offset, block)) => {
                // num_packets += 1;
